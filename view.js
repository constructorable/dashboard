// In script2.js - Verweis auf Modal-Funktionen anpassen

// Alte Funktionen durch Modal-Funktionen ersetzen
function updateChecklistItem(propertyId, encodedItem, isCompleted) {
    // Weiterleitung an Modal-Funktion
...
function getSimpleNotesPreview(notes) {
    // Array-Format (neue Struktur)
    if (Array.isArray(notes)) {
        if (notes.length === 0) {
            return '<em>keine Einträge vorhanden</em>';
        }
        // Neueste Notiz finden
...
// In der Card-Render-Funktion (Beispiel):
// In der renderPropertyCard() oder ähnlichen Funktion:
function renderPropertyCard(property) {
    const typeIcon = getPropertyTypeIcon(property.type);
    const progress = calculateProgress(property.checklist || {});
    const progressColor = getProgressColor(progress);
    
    // EINFACHE Notizen-Vorschau (ohne neue Funktionen)
...
// Hilfsfunktionen für Task-Counting
...
// Modal-Funktionen (falls noch nicht vorhanden)
