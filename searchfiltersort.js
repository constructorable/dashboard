// searchfiltersort.js - Erweitert für Portfolio-Unterstützung
...
let activeFilters = {
    ...
    street: '',
    period: '',
    portfolio: '', // Portfolio-Filter hinzugefügt
    demo: ''
};
...
const sortOptions = {
    ...
    portfolio: { label: 'Portfolio', field: 'portfolio', type: 'string' }, // Portfolio-Sortierung hinzugefügt
...
function initializeSearchFilterSort() {
    console.log('Initialisiere SearchFilterSort...');
    ...
    // Portfolio-Filter sicherstellen
    if (window.portfolioManager) {
        window.portfolioManager.createPortfolioFilter();
    }
    
    renderAllComponents();
    console.log('SearchFilterSort initialisiert');
}
...
function createSidebarItem(property, progress) {
    ...
    // Portfolio in Sidebar-Untertitel hinzugefügt
    ...
    if (Array.isArray(notes)) {
        if (notes.length === 0) {
            return {
                ...
                tooltip: 'Keine Notizen vorhanden'
            };
        }
...
function setupTooltipListeners(card) {
    ...
    tooltipElements.forEach(element => {
        element.addEventListener('mouseenter', startTooltipDelay);
        element.addEventListener('mouseleave', cancelTooltip);
...
function showTooltipDelayed() {
    ...
    if (!text || text === 'Keine Notizen vorhanden' || text === 'Keine Besonderheiten vorhanden') {
        tooltipData = null;
        return;
    }
...
// ... [Alle Modal-Funktionen bleiben unverändert] ...
    ...
    modal.innerHTML = `
        <div class="notes-modal-content">
            <div class="notes-modal-header">
                <h3>
                    <i class="fas fa-sticky-note"></i>
                    Notizen - ${escapeHtml(property.name)}
                </h3>
...
function createDynamicFilters() {
    updateStreetFilter();
    updatePeriodFilter();
    // Portfolio-Filter wird von portfolio.js erstellt
}
...
function resetAllFilters() {
   ...
   activeFilters = {
       ...
       street: '',
       period: '',
       portfolio: '', // Portfolio-Filter zurücksetzen
       demo: ''
   };
...
function resetFilterUI() {
   const selects = ['typeFilter', 'heatingFilter', 'streetFilter', 'periodFilter', 'portfolioFilter', 'demoFilter']; // Portfolio-Filter hinzugefügt
...
function updateFilterUI() {
   const filterMap = {
       ...
       'portfolioFilter': activeFilters.portfolio, // Portfolio-Filter-UI hinzugefügt
...
function showActiveFiltersMessage() {
   ...
   if (activeFilters.portfolio) filterMessages.push(`Portfolio: ${activeFilters.portfolio}`); // Portfolio-Filter-Nachricht
...
function buildPropertySearchData(property) {
   const searchData = {
       name: property.name || '',
       type: property.type || '',
       portfolio: property.portfolio || '', // Portfolio in Suchdata hinzugefügt
   ...
   sortContainer.innerHTML = `
      <select class="sort-select" data-select="sidebar">
       ...
       <div class="sort-header">
          ...
          <button class="btn btn-secondary btn-sm sort-toggle" data-toggle="sidebar" title="Sortierreihenfolge umkehren">
              <i class="fas fa-sort-${currentSortOrder === 'asc' ? 'up' : 'down'}"></i>aufsteigend / absteigend
          </button>
      </div>
  `;
   ...
   sortContainer.innerHTML = `
      <div class="sort-bar">
       ...
       `<option value="${key}" ${key === currentSortBy ? 'selected' : ''}>${option.label}</option>`
              ...
              <button class="btn btn-sm btn-secondary" data-toggle="dashboard" title="Sortierreihenfolge umkehren">
                  <i class="fas fa-sort-${currentSortOrder === 'asc' ? 'up' : 'down'}"></i>
              </button>
              <button class="btn btn-sm btn-secondary" data-action="reset" title="Sortierung zurücksetzen">
...
function handleGlobalChange(e) {
   if (e.target.classList.contains('filter-select') ||
       ['typeFilter', 'heatingFilter', 'streetFilter', 'periodFilter', 'portfolioFilter'].includes(e.target.id)) {  // Portfolio-Filter hinzugefügt
...
function handleFilterChange(e) {
   const filterMap = {
       ...
       'streetFilter': 'street',
       'periodFilter': 'period',
       'portfolioFilter': 'portfolio'  // Portfolio-Filter-Handler hinzugefügt
   };
...
// Globale Funktionen für Window-Zugriff
...
function addModuleStyles() {
   ...
   style.textContent = `
      ...
      /* Modal Styles für Features und Notes */
...
// Debug-Interface für Entwicklung
...
// Globale Funktionen für Modal-Integration
